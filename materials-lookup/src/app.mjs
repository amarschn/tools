...
import {escapeHtml as h, displayUnit, unitLabel, resultText, summarize, summaryText, significant, thicknessText, resultBasis} from './format.mjs';
...
themeSelect.addEventListener('change', () => {
  ...
  try {localStorage.setItem('materials.theme.v1', themeSelect.value);} catch { /* Use the session preference. */ }
});
...
copyLink.addEventListener('click', async () => {
  try {
    ...
    document.querySelector('#live').textContent = 'Link copied.';
  } catch {
    copyLink.textContent = 'Copy address bar link';
    document.querySelector('#live').textContent = 'Copy the link from your browser address bar.';
  }
...
function unitSelect(p) {
  ...
  return `<select data-unit="${h(p.id)}" aria-label="${h(p.name)} unit" title="Override the display unit for ${h(p.name.toLowerCase())}; auto follows the toolbar units."><option value="">${h(automatic)} (auto)</option>${p.units.map(u => `<option value="${h(u.unit)}" ${preference.overrides[p.id] === u.unit ? 'selected' : ''}>${h(unitLabel(u.unit) || 'Ratio')}</option>`).join('')}</select>`;
}
function lineage(id) {
  const out = [];
...
function contextText(o, record) {
  ...
  return Object.entries(conditions).map(([k,v]) => {
    ...
    if (k === 'thickness_m') return `Thickness: ${thicknessText(v, preference.system)}`;
...
function observationHTML(o, p, record) {
  ...
  const basis = resultBasis(o);
  return `<div class="observation" data-observation="${h(o.id)}"><div class="value">${h(resultText(o,unit(p)))}</div><div>${h([contextText(o,record),basis].filter(Boolean).join(' · '))}<details class="observation-details" id="test-${h(o.id)}"><summary>Test details</summary><div>${h(o.test_method?.reported_label || 'Method not specified')}${o.notes ? '<br>' + h(o.notes) : ''}</div></details></div><div class="citation"><span>${h(source.organization)} · p. ${o.source_locator.page}</span><details class="observation-details" id="source-${h(o.id)}"><summary>Source details</summary><div>${h(source.title)}<br>${h(source.revision || '')}<br>${h(o.source_locator.label)}<br><span class="original">Original: ${h(raw.text)} ${h(raw.unit)}</span></div></details></div></div>`;
}
