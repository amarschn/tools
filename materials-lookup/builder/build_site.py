...
any directory:
    ...
    python3 materials/builder/build_site.py

The project root defaults to the parent of ``builder/``.  Use ``--check`` to
...
def _validate_output_location(root: Path, output: Path) -> None:
    ...
    for protected_name in ("curated", "registry", "src", "builder", "tests"):
