...
export function resultBasis(observation) {
  const {kind} = observation.result, basis = observation.basis;
  if (kind === 'lower_bound') return basis === 'typical' ? 'Typical minimum (not guaranteed)' : 'Minimum';
  if (kind === 'upper_bound') return 'Maximum';
  if (kind === 'interval') return basis === 'specified_range' ? 'Specified range' : basis === 'typical' ? 'Typical range' : 'Reported range';
