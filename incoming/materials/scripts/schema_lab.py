...
"""One deterministic command surface for the schema lab.
    ...
    python3 scripts/schema_lab.py migrate    # legacy corpus -> canonical fixture
    python3 scripts/schema_lab.py validate   # structural + semantic validation
    python3 scripts/schema_lab.py check      # migrate to a temp buffer and diff
...
LEGACY_DUMP = REPO_ROOT / "scripts" / "dump_legacy_corpus.js"
...
def command_check(args: argparse.Namespace) -> int:
    ...
    if fresh != stored:
        print(
            ...
            "`python3 scripts/schema_lab.py migrate`",
            file=sys.stderr,
        )
        return 1
